...
%%%%%%%%%%%%%%%%%%%% portland を使って横長の図表を挿入するために・・・
%
\usepackage{portland} % 横長と縦長の変更のため
\usepackage{graphicx} % ページ番号を回転させるため
\makeatletter
\def\labelx#1#2{\@bsphack % 特殊な方法でのラベル生成
  \protected@write\@auxout{}%
         {\string\newlabel{#1}{{#2}{\thepage}}}%
  \@esphack}
\makeatother
%
% コンパイル後に portland.bat を実行して，次の順に pdf ファイルを結合する
% port1.pdf, land1.pdf, port2.pdf, land2.pdf, port3.pdf, land3.pdf
% 結合するには，例えば pdfpdfpdf.com が使える（フリーソフト）
%
%%%%%%%%%%%%%%%%%%%% portland
...
size used here is 9\,pt (10\,pt for English paper).
The length may be within 7 lines. It is
preceded by the title and the authors; both are centered.
The title uses 12\,pt font, while the font size of the authors
is 10\,pt (11\,pt in English paper).}
%
% \titlepagecontrol{1}
%
\receivedate{2012. 10. 30}
...
\onecolumn
\landscape
\thispagestyle{empty} % ページ番号の抑制
\mbox{} % 上下のセンタリングに必要
\vfill

\noindent
 ~\reversemarginpar{\hspace*{-1cm}\rotatebox{-90}{\thepage}} % 欄外にページ番号
% 偶数ページか奇数ページによらず左に出すには \reversemarginpar のようだ・・・
%
% 欄外のマージン記入だけを minipage の左に置いて，幅の 95% に表を書いている
% ページ番号を marginpar に入れるために，無理して table 環境を使わない場合
%
\hfill
\begin{minipage}[c]{.95\textwidth}
\begin{center}
\addtocounter{table}{1} % 表の番号を更新
{\small
{\bf \tablename\nobreak\thetable} 横長の表を無理やり入れたもの
}\\[5mm] % 手動でキャプション
\labelx{tab:landscape}{\thetable} % 手動でラベル作成 \labelx はプリアンブル
% 以下が横長の表
...
\begin{table}  % [p] は付けないこと!!!
\mbox{}        % 図表は上部に配置されるため，高さが小さいときは見かけ上
\vspace*{25mm} % これで少し下に移動できます・・・姑息な手段だが・・・
\begin{center}
...
\vfill % 必要
\noindent
\mbox{}\reversemarginpar{\hspace*{-1cm}\raisebox{.5\textheight}[0pt][0pt]{\rotatebox{-90}{\thepage}}} % 欄外にページ番号
% 一番下の行からページ高さ半分の位置（おおまかに）にページ番号を配置する
% 偶数ページか奇数ページによらず左に出すには \reversemarginpar のようだ・・・
...
% 文章は入れないこと
\noindent
\mbox{}\reversemarginpar{\hspace*{-1cm}\raisebox{.5\textheight}[0pt][0pt]{\rotatebox{-90}{\thepage}}} % 欄外にページ番号　上に持ち上げている
% 偶数ページか奇数ページによらず左に出すには \reversemarginpar のようだ・・・
%
\clearpage
\thispagestyle{empty} % ページ番号の抑制
...
%
\noindent
\mbox{}\reversemarginpar{\hspace*{-1cm}\rotatebox{-90}{\thepage}} % 欄外にページ番号
\hfill
\begin{minipage}[c]{.95\textwidth}
\begin{center}
\addtocounter{table}{1} % 表の番号を更新
...
\labelx{tab:landscape2}{\thetable} % 手動でラベル作成
% 以下が横長の表
\begin{tabular}{|p{6cm}|p{8cm}|p{8cm}|} \hline
見出し1 & 見出し2 & 見出し3 \\ \hline\hline
あいうえお & かきくけこさしすせそ & たちつてとなにぬねのはひふへほ \\ \hline
